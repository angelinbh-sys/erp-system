...
export function CriadoPorInfo({ criadoPorId, criadoPorNome, criadoEm, className = "" }: CriadoPorInfoProps) {
  const [nome, setNome] = useState<string | null>(criadoPorNome ? formatFirstLastName(criadoPorNome) : null);

  useEffect(() => {
    if (criadoPorNome) {
      setNome(formatFirstLastName(criadoPorNome));
      return;
    }
    ...
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data) setNome(formatFirstLastName(data.nome));
      });
  }, [criadoPorId, criadoPorNome]);

  if (!nome && !criadoEm) return null;

  const dataFormatada = criadoEm ? new Date(criadoEm).toLocaleDateString("pt-BR") : null;
  const horaFormatada = criadoEm ? new Date(criadoEm).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) : null;
  ...
  return (
    <div className={`flex items-start gap-2 text-xs text-muted-foreground ${className}`}>
      ...
      <div>
        ...
        {dataFormatada && <span className="ml-1">— {dataFormatada} às {horaFormatada}</span>}
      </div>
    </div>
  );
}
